...
void KKameraConfig::displayGPSuccessDialogue()
{
    ...
    connect(m_deviceSel, &QListView::customContextMenuRequested, this, &KKameraConfig::slot_deviceMenu);
    connect(m_deviceSel, &QListView::doubleClicked, this, &KKameraConfig::slot_configureCamera);
    connect(m_deviceSel, &QListView::activated, this, &KKameraConfig::slot_deviceSelected);
    connect(m_deviceSel, &QListView::clicked, this, &KKameraConfig::slot_deviceSelected);
    ...
    act->setWhatsThis(
        i18n("Click this button to change the configuration of the selected camera.<br><br>The availability of this feature and the contents of the "
    ...
    act->setWhatsThis(
        i18n("Click this button to view a summary of the current status of the selected camera.<br><br>The availability of this feature and the contents of "
             "the Information dialog depend on the camera model."));
    m_toolbar->addAction(act);
    m_toolbar->addSeparator();
...
void KKameraConfig::load()
{
    ...
    for (it = groupList.begin(); it != groupList.end(); it++) {
        if (*it != QStringLiteral("<default>")) {
            ...
            qCDebug(KAMERA_KCONTROL) << "Loading configuration for serial port camera: " << *it;
            kcamera = new KCamera(*it, cg.readEntry("Path"));
            connect(kcamera, qOverload<const QString &>(&KCamera::error), this, qOverload<const QString &>(&KKameraConfig::slot_error));

            connect(kcamera,
                    qOverload<const QString &, const QString &>(&KCamera::error),
                    this,
                    qOverload<const QString &, const QString &>(&KKameraConfig::slot_error));
...
void KKameraConfig::slot_addCamera()
{
    ...
    connect(m_device, qOverload<const QString &>(&KCamera::error), this, qOverload<const QString &>(&KKameraConfig::slot_error));
